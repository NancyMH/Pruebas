    ...
    public String agregar() {
        try{ 
            ...
            if(guardaImagen()) { // Si no ocurrieron errores al tratar de guardar la rutaImagen.
                Puesto puesto = new Puesto(nombre, descripcion, lat, lng, rutaImagen, archivo.getContents()); // Se crea el nuevo puesto.
                puesto.setComidas(new ArrayList<Comida>());
                for (String comida : seleccion) { // Se agregan las relaciones y comida nueva del puesto.
                    puesto.getComidas().add(new Comida(comida)); // Se crea la relación en la tabla comidaPuesto.
                }
